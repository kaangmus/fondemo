...
class ContentPageController extends Controller
{
    ...
    public function store(StoreContentPageRequest $request)
    {
        ...
        if ($request->input('video', false)) {
            $contentPage->addMedia(storage_path('tmp/uploads/' . $request->input('video')))->toMediaCollection('video');
        }
    ...
    public function update(UpdateContentPageRequest $request, ContentPage $contentPage)
    {
        ...
        if ($request->input('video', false)) {
            if (!$contentPage->video || $request->input('video') !== $contentPage->video->file_name) {
                $contentPage->addMedia(storage_path('tmp/uploads/' . $request->input('video')))->toMediaCollection('video');
            }
        } elseif ($contentPage->video) {
            $contentPage->video->delete();
        }
