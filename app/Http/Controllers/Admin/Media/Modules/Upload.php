        ...
        foreach ($request->file as $one) {
            if ($this->allowUpload($one)) {
                ...
                try {
                    ...
                    $metadata = $this->getExif( $one->getRealPath() );
                    $Loaction  = $this->getPlace($metadata['exif']);
                    // $iptc  = $this->getIPTC($one->getRealPath());
                    // $file_exif_data  = exif_read_data('http://nico.nhs/storage/2019-04BAH_6L1A5043.jpg', 0, true);
                    $Place = Place::where('name',$metadata['iptc']['country'])->first();
                    $Year = Year::where('name',$metadata['iptc']['PhotoSource'])->first();
                    $MediaUpload = MediaUpload::where('name',$final_name)->first();
                    $MediaUpload->name = $final_name;
                    $MediaUpload->place_id = $Place->id ?? 0;
                    $MediaUpload->year_id = $Year->id ?? 0;
                    $MediaUpload->metadata = json_encode($metadata);
                    ...
                    $result[]  = [
                        ...
                        'req' => $request->file,
                        'path' => $full_path,
                        'custom_attr' => $request->custom_attrs,
                        'metadata' => $metadata,
                        'location'    => 'dd'
                        // 'iptc'    => $iptc,
                        
                        // 'options' => $file_exif_data,
                    ];
                } catch (Exception $e) {
                    $result[] = [
                        'success' => false,
