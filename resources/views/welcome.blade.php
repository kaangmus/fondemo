    ...
    <div class="swiper-wrapper">
      ...
      <div class="swiper-slide">
        ...
        <div class="slider_content bannercaption{{ $slider->id}}">
          ...
          </span> @if($slider->description)
          <h4 id="fontsize">{{ $slider->description ?? '' }}</h4>@endif @if($slider->btn_text)
...
<section id="team" class="member bg_black text-white">
  <div class="container ">
    <div class="row">
      ...
      <div class="tab">
        <h2 class="maincolor">who we are</h2> @foreach($whoweares as $key=>$whoweare)
        ...
        <div id="who{{$whoadvisor->id}}" class="tabcontent">
          <div class="who">
            <div class="box col">
              <div class="img">
                @if($whoadvisor->photp)
                <img src="{{$whoadvisor->photp->getUrl()}}" alt="{{$whoadvisor->name}}" class="whoadvisor_photo"> @endif
              </div>
              <div class="whoheader">
                <h3 class="text-light">{{$whoadvisor->name}}</h3> {!!$whoadvisor->level!!} </p>
                <div class="who-we-social">
                  @if($whoadvisor->facebook) <a href="{{$whoadvisor->facebook}}" target="_blank"><i
                      class="fab fa-facebook-square"></i></a>@endif @if($whoadvisor->twitter)
                  <a href="{{$whoadvisor->twitter}}" target="_blank"><i class="fab fa-twitter-square"></i></a>@endif
                  @if($whoadvisor->instagram) <a href="{{$whoadvisor->instagram}}" target="_blank"><i
                      class="fab fa-instagram"></i></a>@endif @if($whoadvisor->website)
                  <a href="{{$whoadvisor->website}}" target="_blank"><i class="fas fa-globe"></i></a>@endif
                  @if($whoadvisor->email)
                  <a href="mailto:{{$whoadvisor->email}}" target="_blank"><i
                      class="fas fa-envelope-square"></i></a>@endif
                </div>
      ...
      <div class="lab">
        <h2 class="maincolor">ADVISERS</h2> @foreach($advisors as $key=>$advisor)
        <button class="tablinks"
          onclick="openCity(event, 'who{{$advisor->id}}')"><strong>{{$advisor->name}}</strong></button>
        @endforeach
      </div>
    </div>
  </div>
</section>
{{-- end who we are --}} {{-- slide showcontent --}}
...
@php $data = file_get_contents ('http://hakimages.com/ws_brochures.php'); $json = json_decode($data);
$jsons=array_slice($json, -11, 11, true); @endphp
...
{{-- end grands --}} {{-- exivation timeline --}} @include('exhibition') {{-- exbition timeline --}}
{{-- google map --}}
...
<div id="grants_homeid" class="grants_home">
  <a href="javascript:void(0)" class="search map">
    ...
    <div class="toped">
      ...
      <h2 class="sliderfont text-light text-center text_align18 ">Search and purchase photos by country / by year / by
        category</h2>
