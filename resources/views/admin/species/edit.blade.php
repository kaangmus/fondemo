    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.species.update", [$species->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $species->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                @endif
                <span class="help-block">200px * 200px only</span>
            </div>
           <div class="form-group">
                <label for="published_at">published_at</label>
                <input class="form-control date {{ $errors->has('published_at') ? 'is-invalid' : '' }}" type="text"
                    name="published_at" id="published_at" value="{{ old('published_at', $species->published_at) }}">
                @if($errors->has(''))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                    ...
                    <option value="{{ $key }}" {{ old('type', $species->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
