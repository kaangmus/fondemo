    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.ngos.update", [$ngo->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $ngo->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('videolink') ? 'is-invalid' : '' }}" type="text" name="videolink" id="videolink" value="{{ old('videolink', $ngo->videolink) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{{ old('description', $ngo->description) }}</textarea>
            ...
            <div class="form-group">
                <div class="card">
                    <div class="card-body">
                        ...
                        <select require multiple="multiple" class="form-control select-multiple  {{ $errors->has('species') ? 'is-invalid' : '' }}" name="species[]" id="species">
                            @foreach($species as $id => $species)
                            <option value="{{ $id }}"
                                {{ (in_array($id, old('species', [])) || $ngo->species->contains($id)) ? 'selected' : '' }}>{{ $species }}
                            </option>
...
<script>
    ...
    jQuery(document).ready(function() {
        ...
        $.each(ngoprice,function(key,items){
            let Cloneitems = _.clone(defaultObj);
            // console.log(_.clone(defaultObj));
            var specie = $select2.find('option[value="'+key+'"]').text();
            ngoprice[key] = $.each(Cloneitems,function(Itemkey,item){
                // item.specie = specie;
                // console.log(Itemkey)
                let Newitem = {
                        index: item.index,
                ...
                Cloneitems[Itemkey] = Newitem;
                if( ngoprice[key][Itemkey] == undefined){
   
                    // console.log(item);
                    
                }else if(ngoprice[key][Itemkey].index != Itemkey){
                    Cloneitems[Itemkey] = Newitem;
                    Cloneitems[ngoprice[key][Itemkey].index -1] = ngoprice[key][Itemkey];
                    // delete ngoprice[key][Itemkey];
                }
                // ngoprice[key][Itemkey].specie = specie;
                // ngoprice[key][Itemkey]
                // console.log(ngoprice[key][Itemkey].index);
                // console.log(key);
