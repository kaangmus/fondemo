...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.exhibitionGallery.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.exhibition-galleries.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('exhibition_category') ? 'is-invalid' : '' }}" name="exhibition_category_id" id="exhibition_category_id" required>
                    @foreach($exhibition_categories as $id => $exhibition_category)
                        <option value="{{ $id }}" {{ old('exhibition_category_id') == $id ? 'selected' : '' }}>{{ $exhibition_category }}</option>
                    @endforeach
                </select>
